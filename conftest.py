...
def pytest_configure(config):
    ...
    log.info(f"API Base URL: {settings.api.full_base_url}")
    log.info(f"UI Base URL: {settings.ui.base_url}")
    log.info("=" * 60)
...
def api_base_url():
    ...
    return settings.api.full_base_url


@pytest.fixture(scope="session")
def ui_base_url():
    """Фикстура для получения базового URL UI"""
    return settings.ui.base_url
