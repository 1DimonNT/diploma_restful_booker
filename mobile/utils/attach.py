...
def add_xml(browser):
    ...
    allure.attach(body=xml_dump, name="Page Source (XML)", attachment_type=allure.attachment_type.XML)
...
def add_video(session_id, login, access_key):
    ...
    try:
        ...
        allure.attach(f"Failed to get video: {e!s}", name="Video Error", attachment_type=allure.attachment_type.TEXT)
